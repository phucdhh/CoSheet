    ...
    return this.client({
      '/player/main.js': function () {
        ...
        doPlay = function () {
          ...
          // Initialize user color tracking
          if (!SocialCalc._userColors) {
            SocialCalc._userColors = {};
          }

          // Generate a unique, vibrant color for a user
          SocialCalc.getUserColor = function (username) {
            // If color already assigned, return it
            if (SocialCalc._userColors[username]) {
              return SocialCalc._userColors[username];
            }
            ...
            // Get number of users already assigned colors
            const userCount = Object.keys(SocialCalc._userColors).length;

            // If we've used all palette colors, generate a random one
            let color;
            if (userCount < colorPalette.length) {
              color = colorPalette[userCount];
            } else {
              // Generate a random vibrant color using HSL
              const hue = (userCount * 137.5) % 360; // Golden angle for distribution
            ...
            // Store the color
            SocialCalc._userColors[username] = color;

            // Create valid CSS class name (prefix _ and replace . with -)
            const safeUsername = '_' + username.replace(/\./g, '-').replace(/[^a-zA-Z0-9-]/g, '');
            const styleId = 'user-color-' + safeUsername;

            if (!document.getElementById(styleId)) {
              const style = document.createElement('style');
              style.id = styleId;
              style.textContent = `
                .${safeUsername}.defaultPeer {
                  box-shadow: inset 0 0 0 2px ${color} !important;
                  -webkit-box-shadow: inset 0 0 0 2px ${color} !important;
                  -moz-box-shadow: inset 0 0 0 2px ${color} !important;
                }
              `;
              ...
              switch (this.data.type) {
                ...
                case 'ecells':
                  ...
                  for (user in ref2$ = this.data.ecells) {
                    ...
                    // Assign unique color to this user
                    var userColor = SocialCalc.getUserColor(user);
                    // Update user avatar
                    if (window.UserAvatars) {
                      window.UserAvatars.addUser(user, userColor);
                    }
                    // Use same safe class name as in getUserColor
                    var safeUser = '_' + user.replace(/\./g, '-').replace(/[^a-zA-Z0-9-]/g, '');
                    peerClass = " " + safeUser + " defaultPeer";
                ...
                case 'ecell':
                  // Assign unique color to this user
                  var userColor = SocialCalc.getUserColor(this.data.user);
                  // Update user avatar and activity
                  if (window.UserAvatars) {
                    window.UserAvatars.addUser(this.data.user, userColor);
                    window.UserAvatars.updateActivity(this.data.user);
                  }
                  // Use same safe class name as in getUserColor
                  var safeUser = '_' + this.data.user.replace(/\./g, '-').replace(/[^a-zA-Z0-9-]/g, '');
                  peerClass = " " + safeUser + " defaultPeer";
                  find = new RegExp(peerClass, 'g');
            ...
            ref1$.push({
              ...
              onclick: function () {
                if (window.GraphOnClick) {
                  window.GraphOnClick.apply(window, arguments);
                }
                setTimeout(function () {
                  var ref$;
                  return (ref$ = window.GraphLayout) != null ? ref$.initOnTabClick() : void 8;
                }, 100);
              },
              onclickFocus: true
            });
