...
(function(){
  ...
  this.include = function(){
    ...
    hmac = !KEY
      ...
      : function(it){
        var encoder;
        return HMAC_CACHE[it] || (HMAC_CACHE[it] = (encoder = require('crypto').createHmac('sha256', Buffer.from(KEY)), encoder.update(it.toString()), encoder.digest('hex')));
      };
    ...
    ExportJ = function(type){
      return api(function(it){
        ...
        for (i$ = 0, len$ = saves.length; i$ < len$; ++i$) {
          idx = i$;
          save = saves[i$];
          ref$ = J.read(Buffer.from(save)), harb = ref$[0], Sheet1 = ref$[1].Sheets.Sheet1;
    ...
    this.get((ref$ = {}, ref$[BASEPATH + "/_/:room/csv.json"] = function(){
      var room, ref$, snapshot, lines, result, i$, len$, line, rowNum, urlMatch, titleLine, url, titleMatch, title, this$ = this;
      room = encodeURI(this.params.room);
      this.response.type(Json);
      if ((ref$ = SC[room]) != null && ref$._snapshot) {
        ...
        for (i$ = 0, len$ = lines.length; i$ < len$; ++i$) {
          line = lines[i$];
          if (line.match(/^cell:A(\d+):t:/)) {
            rowNum = parseInt(RegExp.$1);
            if (rowNum > 1) {
              urlMatch = line.match(/^cell:A\d+:t:([^\\n]+)/);
              titleLine = lines.find(fn$);
              if (urlMatch && titleLine) {
                url = urlMatch[1];
        ...
        return this.response.send(200, JSON.stringify(result));
      }
      SC._get(room, IO, function(arg$){
        var snapshot, lines, result, i$, len$, line, rowNum, urlMatch, titleLine, url, titleMatch, title;
        snapshot = arg$.snapshot;
        if (!snapshot) {
          return this$.response.send(404, '');
        }
        ...
        return this$.response.send(200, JSON.stringify(result));
        function fn$(l){
          return l.match("cell:B" + rowNum + ":t:");
        }
      });
      return undefined;
      function fn$(l){
    ...
    requestToSave = function(request, cb){
      ...
      if (request.is('application/json')) {
        ...
        console.log("DEBUG: JSON snapshot length: " + (snapshot != null ? snapshot.length : void 8));
        if (snapshot) {
          return cb(snapshot);
        }
      }
      ...
      return request.on('end', function(){
        var buf, str, wrapped, iconv, k, ref$, save;
        buf = Buffer.concat(cs);
        str = buf.toString('utf8');
        console.log("DEBUG: RAW Body length: " + buf.length + ", Start: " + str.substring(0, 50).replace(/\n/g, '\\n'));
        if (str.indexOf('version:') === 0 || str.indexOf('socialcalc:version:') === 0 || request.is('text/x-socialcalc')) {
    ...
    this.post({
      '/_/create-toc/:room': function(){
        var room, chunks, this$ = this;
        room = encodeURI(this.params.room);
        chunks = [];
        ...
        return this.request.on('end', function(){
          var bodyStr, tocData, tocFilePath, e;
          try {
            bodyStr = Buffer.concat(chunks).toString('utf8');
            tocData = JSON.parse(bodyStr);
            if (tocData && Array.isArray(tocData)) {
              tocFilePath = RealBin + "/static/toc-" + room + ".json";
              fs.writeFileSync(tocFilePath, JSON.stringify(tocData), 'utf8');
              return this$.response.send(201, 'OK');
            } else {
              return this$.response.send(400, 'Invalid TOC data - must be array');
            }
          } catch (e$) {
            e = e$;
            return this$.response.send(400, 'JSON parse error');
          }
        });
      }
    });
    this.put({
      '/_/:room': function(){
        var room, this$ = this;
        console.log("DEBUG: PUT request for room: " + this.params.room);
        room = encodeURI(this.params.room);
        this.response.type(Text);
        return requestToSave(this.request, function(snapshot){
          var ref$;
          console.log("DEBUG: Saving snapshot for " + room + ", length: " + (snapshot != null ? snapshot.length : void 8));
          if ((ref$ = SC[room]) != null) {
            ref$.terminate();
          }
          delete SC[room];
    ...
    function fn$(){
      ...
      return this.request.on('end', function(){
        var buf, idx, toc, parsed, sheetsToIdx, res, k, Sheet1, todo, save, tocArray, tocFilePath;
        buf = Buffer.concat(cs);
        idx = 0;
        toc = '#url,#title\n';
        ...
        for (k in parsed) {
          idx++;
          tocArray.push(["/" + this$.params.room + "." + idx, k]);
        }
        tocFilePath = RealBin + "/static/toc-" + room + ".json";
