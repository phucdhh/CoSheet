                ...
                switch (msg.t) {
                    ...
                    case 'socialcalc_multi':
                        ...
                        uploadSequence.then(function () {
                            return uploadSheetData(currentRoom, toc, 'text/csv; charset=utf-8');
                        }).then(function () {
                            // Create TOC JSON file for multi-view
                            var tocArray = [['#url', '#title']];
                            sheets.forEach(function(sheet, index) {
                                tocArray.push(['/' + currentRoom + '.' + (index + 1), sheet.name]);
                            });
                            
                            return fetch('/_/create-toc/' + currentRoom, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify(tocArray)
                            }).then(function(response) {
                                if (!response.ok) throw new Error('Failed to create TOC file: ' + response.status);
                                return response;
                            });
                        }).then(function () {
                            closeDialog(loadingDialog);
